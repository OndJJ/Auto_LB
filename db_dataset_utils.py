...
# COCO 스타일 데이터셋 형식을 삽입 및 사용하기 위한 유틸리티입니다.
...
# from auto_lb.utils import COLOR_LIST


# # 전체적으로 Mongo db와 함께 사용되는 py
# # DB = ID type 확인, dataset 초기화, 인덱스 생성, 카테고리 삽입, 이미지 삽입,
# # 라이센스 확인, 주석 추가, DB 내보내기 등의 역할을 수행하는 파일이다.
...
#             # keypoint에 색상 추가
#             if 'keypoints' in cat and 'keypoints_style' not in cat:   # == 문자열이 가능?
#                 print("\tWARNING: Adding keypoint styles to category: %s" %
#                       (cat['name'],))
#                 keypoints_style = []
...
#             if len(panic) > 0:
#                 raise
#             print("Attempted to insert duplicate categories, %d new categories inserted" % (
...
#             if len(panic) > 0:
#                 raise
#             print("Attempted to insert duplicate licenses, %d new licenses inserted" % (
#                 bwe.details['nInserted'],))
...
# # 'Dataset loading and exporting utilities.'
...
# # database에서 출력 시 주석을 비활성화 / export 작업에 수행 됨
...
#     action = args.action  # args 실행
#     if action == 'drop':
#         drop_dataset(db)
#     elif action == 'load':  # dataset에서 json 파일 load
